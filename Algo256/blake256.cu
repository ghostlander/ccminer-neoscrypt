...
/* Second part (64-80) msg never change, store it */
__device__ static
void blake256_compress_8(uint32_t *h, const uint32_t *block, const uint32_t T0, const int rounds)
{
	uint32_t /*_ALIGN(8)*/ m[16];
	uint32_t v[16];
...
#pragma unroll
		...
		blake256_compress_8(h, ending, 640, rounds);
	...
	uint32_t max_nonce, unsigned long *hashes_done, int8_t blakerounds=14)
{
	...
	uint32_t crcsum;
#endif
	uint32_t intensity = (device_sm[device_map[thr_id]] > 500) ? 28 : 28;
